...
<form name="kota_form" action="<?php echo $form_action;?>" method="post">
    <div  align="center">
    <fieldset>
        <legend>UPDATE DATA KOTA</legend>
        ...
        <td><input type="text" name="propinsi" readonly="readonly" 
                   ...
                   ?>" />
            <input type="hidden" name="propinsiID" 
                   value="<?php echo set_value('propinsiID',  isset($default['propinsiID']) ? $default['propinsiID'] : '');?>" />
        ...
        <td><input type="text" name="namaKota" size="40" 
                   value="<?php
                        $kota = $this->Kota_model->getKotaByID($this->uri->segment(3));
                        echo $kota->namaKota;
                   ?>" />
        </td>
        </tr>
